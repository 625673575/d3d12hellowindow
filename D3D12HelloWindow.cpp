...
void D3D12HelloWindow::LoadPipeline()
{
	...
	// Create root signature
	{
		...
		rangesTex[0].Init(D3D12_DESCRIPTOR_RANGE_TYPE_SRV, 2, 0, 0, D3D12_DESCRIPTOR_RANGE_FLAG_DATA_STATIC);

		CD3DX12_ROOT_PARAMETER1 rootParameters[3];
		rootParameters[0].InitAsDescriptorTable(1, &rangesTex[0], D3D12_SHADER_VISIBILITY_PIXEL);
		rootParameters[1].InitAsConstantBufferView(3, 0, D3D12_ROOT_DESCRIPTOR_FLAG_NONE, D3D12_SHADER_VISIBILITY_VERTEX);
	...
	CreateGridMeshVertexUV(4, m_vertex_num, m_indicies_num, vertices, indicies);
	{
		...
		memcpy(pVertexDataBegin, &vertices[0], VertexDataSize);
		m_vertexBuffer->Unmap(0, nullptr);
		// Initialize the vertex buffer view.
		m_vertexBufferView.BufferLocation = m_vertexBuffer->GetGPUVirtualAddress();
		m_vertexBufferView.StrideInBytes = sizeof(VertexUV);
...
void D3D12HelloWindow::LoadAssets()
{
	...
	// Create the constant buffer.
	{
		...
		memcpy(m_pCbvDataBegin, &chromeData, sizeof(chromeData));
	...
	// Create the texture.
	{
		...
		std::vector<UINT8> texture = GenerateTextureData(2048, 2048);
...
void D3D12HelloWindow::GenerateDistortionTexture()
{
	...
	//create intermediate render target
	{
		...
		CD3DX12_CPU_DESCRIPTOR_HANDLE rtvHandle(m_rtvDescriptorHeap->GetCPUDescriptorHandleForHeapStart(), FrameCount, m_rtvDescriptorSize);//
		...
		CD3DX12_CPU_DESCRIPTOR_HANDLE srvHandle(m_cbvSrvHeap->GetCPUDescriptorHandleForHeapStart(), m_cbvSrvDescriptorSize);
	...
	//Draw to render target
	{
		...
		ThrowIfFailed(m_commandList->Reset(m_commandAllocator.Get(), m_distortionPipelineState.Get()));
...
void D3D12HelloWindow::PopulateCommandList()
{
	...
	m_commandList->SetGraphicsRootConstantBufferView(2, m_constantBuffer->GetGPUVirtualAddress());//root parameter slot 2 is CBChromatic.
	...
	m_commandList->SetGraphicsRootConstantBufferView(1, m_constantBuffer->GetGPUVirtualAddress() + m_constantBufferPerSize);//root parameter slot 1 is CBTimewarp.
